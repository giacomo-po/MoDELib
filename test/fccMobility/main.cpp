...
int main (int argc, char * const argv[])
{
    ...
    //const Eigen::Matrix<double,3,1> m = (Eigen::Matrix<double,3,1>()<<1.0, -1.0,0.0).finished().normalized();
    const Eigen::Matrix<double,3,1> b = (Eigen::Matrix<double,3,1>()<<-1.0, 0.0,1.0).finished().normalized();
...
//    const Eigen::Matrix<double,3,1> xi = s; // screw
    ...
    PeriodicElement<29,Isotropic> Cu;
    
    Cu.dm.velocity(S/W.mu,s,xi,n,t)
