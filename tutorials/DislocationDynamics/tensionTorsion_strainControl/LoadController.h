...
//        const unsigned int userOutputColumn=DN.userOutputColumn();
        ...
        maxTwist_Rad=TextFileParser("./loadInput.txt").readScalar<double>("maxTwist_Rad",true);
...
//                last_update_time=iter->second(0);
    ...
    void addDirichletConditions(const DislocationNetworkType& DN)
    {
        ...
        if(DN.simulationParameters.runID>=relaxSteps)
        {
            if (apply_torsion)
            {
                u.addDirichletCondition(nodeList_top,*this,{1,1,1}); // prescribe only u3
            }
            else
            {
                                                   ...
                                                   Eigen::Matrix<double,dofPerNode,1>& val) const
    {
        ...
        val=(Eigen::Matrix<double,3,1>()<<(rot*(node.P0-pivot)-(node.P0-pivot)).template segment<2>(0),initialDisplacement+deltaDisplacement).finished();
        return val;
    }
    ...
    {/*!@param[in] DN the DislocationNetwork
      ...
      */
        if(DN.simulationParameters.runID>=relaxSteps)
        {
            ...
            if(initialTwist_Rad+deltaTheta>maxTwist_Rad)
            {
                       ...
                       std::ofstream& F_labels) const
    {
        ...
        double avgDispZ(dispZ/fieldPoints.size());
        // COMPUTATION OF LOAD
        ...
        loadedBnd.integrate(DN.bvpSolver.get(),force,&BvpSolverType::bvpTraction);   // integrate bvp moment about tt.pivot
        loadedBnd.integrate(DN.bvpSolver.get(),force,&BvpSolverType::ddTraction,DN); // integrate  dd moment about tt.pivot

        
//        std::stringstream os;
        f_file<<" "<<avgRotZ<<" "<<" "<<torque(2)<<" "<<avgDispZ<<" "<<force(2);
//        return os.str();
