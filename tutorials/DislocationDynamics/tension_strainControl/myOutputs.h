...
//    double  rotZ = 0.0;
    ...
    Tensioner tt(DN.shared.bvpSolver.finiteElement());
    
    // Obtain a list of nodes under the Tensioner
    const size_t nodeListID=DN.shared.bvpSolver.finiteElement().template createNodeList<Tensioner>();
...
//    avgRotZ =  rotZ/DN.shared.bvpSolver.finiteElement().nodeList(nodeListID).size();
    ...
    /******************************************************************************/
    // COMPUTATION OF LOAD
    auto loadedBnd = Tensioner::boundary(DN.shared.bvpSolver.finiteElement());
