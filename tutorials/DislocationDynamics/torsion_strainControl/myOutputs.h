...
//    double dispZ = 0.0;
    ...
    Torsioner tt(DN.shared.bvpSolver.finiteElement());
    
    // Obtain a list of nodes under the Torsioner
    const size_t nodeListID=DN.shared.bvpSolver.finiteElement().template createNodeList<Torsioner>();
...
//    avgDispZ = dispZ/DN.shared.bvpSolver.finiteElement().nodeList(nodeListID).size();
    ...
    /******************************************************************************/
    // COMPUTATION OF LOAD
    auto loadedBnd = Torsioner::boundary(DN.shared.bvpSolver.finiteElement());
