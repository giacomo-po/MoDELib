...
/**************************************************************************/
/**************************************************************************/
struct CylindricalPunchNodeSelector
{

    const double punchDiameter;
    ...
    CylindricalPunchNodeSelector(const FiniteElementType& fe,
                      const double& punchDiameter_in,
                      const double& zFace_in,
                      const double& centerX_in,
                      const double& centerY_in,
                      const double& geometricTol) :
    /* init */ punchDiameter(punchDiameter_in)
    /* init */,zFace(zFace_in)
...
//        return (   std::pow(node.P0(0)-centerX,1)+(0.5*punchDiameter+geometricTol)
//                /*   */ && std::fabs(node.P0(1)-centerY)<(0.5*punchDiameter+geometricTol)
//                /*   */ && std::fabs(node.P0(2)-zFace)<geometricTol);
        
        return (std::pow(node.P0(0)-centerX,2)+std::pow(node.P0(1)-centerY,2)<std::pow(0.5*punchDiameter+geometricTol,2))
        /*  */ && std::fabs(node.P0(2)-zFace)<geometricTol;
...
struct LoadController
{
    ...
    /* init list */,punchDiameter(TextFileParser("./inputFiles/loadControllerInput.txt").readScalar<double>("punchDiameter",true))
    ...
    /* init list */,nodeList_top(u.fe().template createNodeList<CylindricalPunchNodeSelector>(punchDiameter,zFace,centerX,centerY,geometricTol))
    ...
    void update(const DislocationNetworkType& DN)
    {
        ...
        if(DN.simulationParameters.runID>=relaxSteps)
        {
            ...
            std::cout<<"LoadController: imposed displacement = "<<initialDisplacement+deltaDisplacement<<std::endl;
        }
    }
    ...
    IntegrationDomain<FiniteElementType,1,3,GaussLegendre> topBoundary()
    {
        ...
        CylindricalPunchNodeSelector nodeSelector(u.fe(),punchDiameter,zFace,centerX,centerY,geometricTol);
