...
b=s*B(r,:)/norm(B(r,:));    % the Burgers vector of the current loop 
Nc=[];
...
P(:,1)=P1+[R R 0]'; % shift the axis to match mesh file
u=rand(1);
...
if (   norm(P([1 2],1)-[R R]')<R && P(3,1)>0 && P(3,1)<H ...
    ...
    && norm(P([1 2],4)-[R R]')<R && P(3,4)>0 && P(3,4)<H) % all points are inside
