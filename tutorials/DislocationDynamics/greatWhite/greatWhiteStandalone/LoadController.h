...
struct LoadController
{
    ...
    void init(const DislocationNetworkType& DN)
    {
        ...
        pressure=TextFileParser("./inputFiles/loadControllerInput.txt").readScalar<double>("pressure",true);
        ...
        Eigen::Matrix<double,3,3> p(-pressure*Eigen::Matrix<double,3,3>::Identity());
        //    auto p=make_constant(-0.01*Eigen::Matrix<double,3,3>::Identity());
        auto ndA_2=u.fe().template boundary<ExternalBoundary,3,GaussLegendre>();
        auto lWF_2=(test(u),make_constant(p))*ndA_2;
        pGV=lWF_2.globalVector();
    ...
    {
        std::cout<<"Updating LoadController: applied stress="<<initialStress+deltaStress<<std::endl;
