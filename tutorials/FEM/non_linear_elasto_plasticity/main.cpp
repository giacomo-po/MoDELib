...
struct StiffnessMatrix : public EvalFunction<StiffnessMatrix<FiniteElementType,dim> >
{
    ...
    const TrialFunction<FiniteElementType,'Fp',dim*dim>& fp;       // displacement field
    ...
    {/*!@param[in] elem the element
      ...
      */
        ...
        for(int i=0;i<dim;++i)
        {
            for(int J=0;J<dim;++J)
            {
                const voigtRow=tensor2toVoigt(i,J);
                for(int k=0;k<dim;++k)
                {
                    for(int L=0;L<dim;++L)
                    {
                        const voigtCol=tensor2toVoigt(k,L);
                        ...
                        for(int a=0;a<dim;++a)
                        {
                            for(int b=0;b<dim;++b)
                            {
                                for(int g=0;g<dim;++g)
                                {
                                    for(int d=0;d<dim;++d)
                                    {
                                        temp(voigtRow,voigtCol)+= C(a,b,g,d)*Fe(i,a)*Gp(J,b)*Fe(k,g)*Gp(L,d);
                                    }
                                }
                            }
                        }
