...
namespace model
{
    ...
    public:
        ...
        typedef std::multimap<int,MeshBoundarySegment<dim>> LineSegmentContainerType;
    ...
    public:
        ...
        void updateWithMeshPlane(const MeshPlane<dim>& gp)
        {
            if(this->size())
            {
                ...
                {// gp is a region-boundary plane. We need to select the current bounding lines which are also on gp
                    BoundingLineSegments<dim> temp;
                    for(const auto& seg : *this)
                    {
                        const bool P0contained(gp.contains(seg.second.P0));
                        const bool P1contained(gp.contains(seg.second.P1));
                        if(P0contained && P1contained)
                        {
                            if(seg.second.hasZeroLength())
                            {
                                if(isUniquePoint(seg.second.center()))
                                {
                                    temp.emplace(seg.first,seg.second);
                                }
                            }
                            else
                            {
                        ...
                        {
                            if(isUniquePoint(seg.second.P0))
                            {
                                temp.emplace(seg.first,MeshBoundarySegment<dim>(seg.second.P0,seg.second.P0,seg.second.face));
                            }
                        }
                        else if(P1contained)
                        {
                            if(isUniquePoint(seg.second.P1))
                            {
                                temp.emplace(seg.first,MeshBoundarySegment<dim>(seg.second.P1,seg.second.P1,seg.second.face));
                            }
                        }
