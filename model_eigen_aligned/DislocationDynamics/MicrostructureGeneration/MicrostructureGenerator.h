...
//        const double averageLoopSize;
        ...
        //        /* init*/,inclusionsMobilityReduction(TextFileParser("./inputFiles/initialMicrostructure.txt").readArray<double>("inclusionsMobilityReduction",true))
        {
            ...
            bool testSuperGlidePlane(true);
            if(testSuperGlidePlane)
            {
                std::cout<<"testSuperGlidePlane"<<std::endl;

                
                    GlidePlaneFactory<dim> gpF(poly);
                for(const auto& loop : loopsIO)
                {
                    if(loop.loopType==DislocationLoopIO<3>::GLISSILELOOP)
                    {
                    GlidePlaneKey<3> referencePlaneKey(loop.grainID,loop.P,poly.grain(loop.grainID).reciprocalLatticeDirection(loop.N));
                    std::shared_ptr<GlidePlane<3>> referencePlane(gpF.get(referencePlaneKey));
                    PeriodicGlidePlane pgp(gpF,referencePlane->key,loop.B);
                    
                        DDauxIO<dim> auxIO;
                        auxIO.writeTxt(gpF,0);

                        
                    for(auto& node : nodesIO)
                    {
                        node.P+=VectorDimD::Random().cross(loop.N)*500.0;
                    }
                        
                        writeConfigFiles(1);
                    
                    pgp.addPatchesContainingPolygon(nodesIO);
                    
                    std::cout<<"pgp.isCompact() "<<pgp.isCompact()<<std::endl;
                    }
                }
                
                    DDauxIO<dim> auxIO;
                    auxIO.writeTxt(gpF,1);
            }
        }
                            ...
                            if(radius>0.0)
                            {
                                ...
                                if(allPointsInGrain(points,grainID))
                                {
                                    ...
                                    {// insert loops on the six sides
                                        for(int k=0;k<6;++k)
                                        {// inser lateral loops
                                            nodesIO.emplace_back(nodeID+k+6,points[k],Eigen::Matrix<double,1,3>::Zero(),1.0,snID,0);
                                            const int nextNodeID=(k+1)<6? nodeID+k+1 : nodeID;
                                            edgesIO.emplace_back(loopID,nodeID+k,nodeID+k+6,0);
                                            edgesIO.emplace_back(loopID,nodeID+k+6,nextNodeID+6,0);
