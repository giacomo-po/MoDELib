...
namespace model
{
    struct DDconfigVtk : public DDconfigIO<3>
    {
...
//            assert(loopEdgeMap.size()==this->loopLinks().size());
            ...
            for(const auto& loop : this->loops())
            {
                ...
                auto vIter0(this->loopNodeMap().find(loopFound->second.begin()->first)); //source
                assert(vIter0!=this->loopNodeMap().end() && "VERTEX NOT FOUND IN V-FILE");
                nodePositions.push_back(vIter0->second->P);
                for(size_t k=0;k<loopFound->second.size();++k)
                {
                    const auto nodeFound=loopFound->second.find(nodeIDs.back());
                    if(k<loopFound->second.size()-1)
                    {
                        nodeIDs.push_back(nodeFound->second);
                        
                        auto vIter(this->loopNodeMap().find(nodeFound->second)); //source
                        assert(vIter!=this->loopNodeMap().end() && "VERTEX NOT FOUND IN V-FILE");
                ...
                if(fabs(loop.B.dot(loop.N))<FLT_EPSILON && loop.N.squaredNorm()>FLT_EPSILON)
                {
                    ...
                    for(const auto& triID : tri)
                    {
                        ...
                        const size_t ptID0=std::distance(this->loopNodeMap().begin(),this->loopNodeMap().find(nodeID0));
                        const size_t ptID1=std::distance(this->loopNodeMap().begin(),this->loopNodeMap().find(nodeID1));
                        const size_t ptID2=std::distance(this->loopNodeMap().begin(),this->loopNodeMap().find(nodeID2));
        ...
        /**********************************************************************/
        DDconfigVtk(const size_t& frameID,
        ...
        /* init */ loopPoints( vtkSmartPointer<vtkPoints>::New()),
        ...
        /* init */ triangleActor(vtkSmartPointer<vtkActor>::New())
        {
            ...
            {
                //                assert(DDconfigIO<3>::isTxtGood(frameID));
        ...
        ~DDconfigVtk()
        {
            ...
            renderer->RemoveActor(loopActor);
...
//            renderer->RemoveActor(gpActor);
        ...
        void plotLoops()
        {
            ...
            {
                const auto& node(loopNodes()[k]);
                loopNodesMap.emplace(node.sID,k);
                
                loopPoints->InsertNextPoint(node.P(0),
                                            node.P(1),
                                            node.P(2));
            }

            
            loopPolyData->Allocate();
            size_t connectivityID=0;
            for(const auto& link : this->loopLinks())
            {
                const auto sourceIter(loopNodesMap.find(link.sourceID));
                assert(sourceIter!=loopNodesMap.end());
                const auto sinkIter(loopNodesMap.find(link.sinkID));
                assert(sinkIter!=loopNodesMap.end());
                
                vtkIdType connectivity[2]; //points IDs of each line segment
                connectivity[0] = sourceIter->second;
                connectivity[1] = sinkIter->second;
                loopPolyData->InsertNextCell(VTK_LINE,2,connectivity);
                connectivityID+=2;
            }
            loopPolyData->SetPoints(loopPoints);
            loopMapper->SetInputData(loopPolyData);
            loopActor->SetMapper(loopMapper);
            loopActor->GetProperty()->SetLineWidth(2.0);
            loopActor->GetProperty()->SetColor(0.0,0,1.0);
//            loopActor->GetProperty()->SetOpacity(glidePlaneOpacity);
//            loopActor->SetVisibility(showLoops);
    ...
    // Static data members
    float DDconfigVtk::tubeRadius=5.0;
    bool DDconfigVtk::scaleRadiusByBurgers=false;
    float DDconfigVtk::alpha=0.5;
    DDconfigVtk::ColorScheme DDconfigVtk::clr=DDconfigVtk::colorBurgers;
    size_t DDconfigVtk::Np=2;
    bool DDconfigVtk::showBoundarySegments=false;
    bool DDconfigVtk::blackGrainBoundarySegments=false;
    bool DDconfigVtk::showVelocities=false;
    bool DDconfigVtk::showNodeIDs=false;
    float DDconfigVtk::velocityFactor=100.0;
    bool DDconfigVtk::showZeroBuergers=false;
    bool DDconfigVtk::showSingleNode=false;
    size_t DDconfigVtk::singleNodeID=0;
