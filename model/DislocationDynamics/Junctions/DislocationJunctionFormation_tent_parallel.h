...
#include <model/Threads/SymInteractionDeque.h>
...
//                                        const double avoidNodeIntersection=0.2;
            ...
            SymInteractionDeque<typename DislocationNetworkType::NetworkLinkContainerType::const_iterator> symDeq(DN.linkBegin(),DN.linkEnd());
...
#endif
            for (size_t inter=0;inter<symDeq.size();++inter)
            {
                const typename NetworkLinkContainerType::const_iterator& linkIterA(symDeq[inter].first);
                const typename NetworkLinkContainerType::const_iterator& linkIterB(symDeq[inter].second);
