...
//            typename SimplexObserverBase<SimplexObserver<dim>,dim,0>::SimplexIDType last;
            ...
            for (int k=0;k<dim;++k)
            {
//                typename SimplexObserverBase<SimplexObserver<dim>,dim,0>::SimplexIDType current;
                typename SimplexTraits<dim,0>::SimplexIDType current;
                current<<s.xID(k); // a 1x1 matrix
                temp.col(k) << (s.mesh->template observer<0>().pSimplex(current))->P0 - (s.mesh->template observer<0>().pSimplex(last))->P0;
            }
            return temp;
        }
...
//            typename SimplexObserverBase<SimplexObserver<dim>,dim,0>::SimplexIDType last;
            ...
            for (int n=0;n<nodesPerElement;++n)
            {
                ...
                const Eigen::Matrix<double,dim,1> P(FL*BarycentricTraits<dim>::l2x(baryNodalCoordinates.row(n))+(s.mesh->template observer<0>().pSimplex(last))->P0);
