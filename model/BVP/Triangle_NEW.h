...
#include <mmdl/BVP/commonTypeDefs.h>
		...
		Eigen::Matrix<double,dim,3> getTriInfiniteForce (const T* const pt) {
		  ...
		  mmdl::Quadrature<2,qOrder>::integrate(this,tractionInt,&Triangle::dislocationStressKernel<PointerWrapper<T> > , pts, triN, tractionMatrix);
		...
		//  VectorDim n, v1 , v2;
		  ...
		  VectorDim v1 ( this->eleNodes[1]->Pc - this->eleNodes[0]->Pc);
		  VectorDim v2 ( this->eleNodes[2]->Pc - this->eleNodes[0]->Pc);
		  
		//  VectorDim n ( v2.cross(v1));
		...
		*/
		...
		Eigen::Matrix<double,dim,3> getTriInfiniteForce_gp (const T* const pt) {
		  ...
		  else mmdl::Quadrature<2,3>::integrate(this,tractionInt,&Triangle::dislocationStressKernel<PointerWrapper<T> > , pts, outNormal, tractionMatrix);
		...
		Eigen::Matrix<double,dim,3> subTri_getTriInfiniteForce (const T* const pt , Eigen::Matrix<double,dim,dim> subTriPoints, unsigned int iSeg) {
		  ...
		  mmdl::Quadrature<2,qOrder>::integrate(this,tractionInt,&Triangle::subTri_dislocationStressKernel<PointerWrapper<T> > , pts, triN, tractionMatrix, subTriPoints , iSeg );
...
// 			}
		...
		*/
		...
		Eigen::Matrix<double,dim,1> getTriInfiniteTraction (const T* const pt) const {
		  ...
		  else mmdl::Quadrature<2,qOrder>::integrate(this,tractionInt,&Triangle::dislocationStressKernel<PointerWrapper<T> > , pts, triN);
