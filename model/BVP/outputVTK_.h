...
void writeGND(Eigen::Vector3d box,unsigned int npnts){
  ...
  for (unsigned int k=1; k<npnts ; k++){
    for (unsigned int j=1; j<npnts ; j++){
      for (unsigned int i=1; i<npnts ; i++) {
	vindx [i][j][k] = iv;
  ...
  for (unsigned int k=0; k<npnts ; k++){
	...
	if(!isT.first) assert(0 &&"Error in GND calculations. Sample point outside domain");
  ...
  /*
  for (unsigned int k=1; k<npnts-1 ; k++){
    for (unsigned int j=1; j<npnts-1 ; j++){
...
void writeVTK_file (const T* const pT) {
  ...
  Eigen::Vector3d box(20000.0e00, 20000.0e00, 20000.0e00);
  unsigned int npnts = 51;
