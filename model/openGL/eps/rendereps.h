...
void
outputEPS(int size, int doSort, const std::string& filename)
{
...
//    GLfloat *feedbackBuffer;
    ...
    std::cout<<"size4="<<size<<std::endl;

    feedbackBuffer = calloc(size, sizeof(GLfloat));
    std::cout<<"size3="<<size<<std::endl;

    glFeedbackBuffer(size, GL_3D_COLOR, (GLfloat*) feedbackBuffer);
 ...
 //   if (filename) {
        ...
        if (file) {
            spewWireFrameEPS(file, doSort, 0, (GLfloat*) feedbackBuffer, "rendereps");
...
//    } else {
        /* Helps debugging to be able to see the decode feedback
         buffer as text. */
 //       printBuffer(returned, (GLfloat*) feedbackBuffer);
  //  }
    free(feedbackBuffer);
}
