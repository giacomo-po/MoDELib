...
//			std::cout<<"Size of i is: "<<Vi.second->outNeighborhood().size()+Vi.second->inNeighborhood().size()<<std::endl;
//			std::cout<<"Size of j is: "<<Vj.second->outNeighborhood().size()+Vj.second->inNeighborhood().size()<<std::endl;
//			
//			
//			const size_t newID(VertexInsertion<VertexType>(networkVertexMapRef).insert(NodeInput...)); // CHANGE THIS LIKE EXPAND
//			
//			for (typename NeighborContainerType::const_iterator nIter=Vi.second->outNeighborhood().begin(); nIter!=Vi.second->outNeighborhood().end();++nIter){
//				const size_t k(boost::tuples::get<0>(nIter->second)->sID);
//				if (k!=j){
//					VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).connect(newID,k,boost::tuples::get<1>(nIter->second)->flow);
//				}
//			}
//			for (typename NeighborContainerType::const_iterator nIter=Vi.second->inNeighborhood().begin(); nIter!=Vi.second->inNeighborhood().end();++nIter){
//				const size_t k(boost::tuples::get<0>(nIter->second)->sID);
//				if (k!=j){
//					VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).connect(k,newID,boost::tuples::get<1>(nIter->second)->flow);
//				}
//			}
//			
...
//			for (typename NeighborContainerType::const_iterator nIter=Vj.second->outNeighborhood().begin(); nIter!=Vj.second->outNeighborhood().end();++nIter){
//				const size_t k(boost::tuples::get<0>(nIter->second)->sID);
//				assert(k!=i && "VERTEX i HAS JUST BEEN REMOVED");
//				
//				const FlowType fjk(boost::tuples::get<1>(nIter->second)->flow);
//				const isConstNetworkEdgeType eNk(EdgeFinder<EdgeType>(networkEdgeMapRef).link(newID,k));
//				if (eNk.first){
//					const FlowType fNk(eNk.second->flow);
//					VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).template disconnect<0>(newID,k);
//					if((fNk+fjk).norm()>FLT_EPSILON){
//						VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).connect(newID,k,fNk+fjk);
//					}
//				}
//				else{
//					const isConstNetworkEdgeType ekN(EdgeFinder<EdgeType>(networkEdgeMapRef).link(k,newID));
//					if(ekN.first){
//						const FlowType fkN(ekN.second->flow);
//						VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).template disconnect<0>(k,newID);
//						if((fkN-fjk).norm()>FLT_EPSILON){
//							VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).connect(k,newID,fkN-fjk);
//						}
//					}
//					else {
//						VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).connect(newID,k,fjk);
//						
//					}					
//				}
//			}
...
//					const FlowType fNk(eNk.second->flow);
//					VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).template disconnect<0>(newID,k);
//					if((fNk-fkj).norm()>FLT_EPSILON){
//						VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).connect(newID,k,fNk-fkj);
//					}
//				}
//				else{
//					const isConstNetworkEdgeType ekN(EdgeFinder<EdgeType>(networkEdgeMapRef).link(k,newID));
...
//						const FlowType fkN(ekN.second->flow);
//						VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).template disconnect<0>(k,newID);
//						if((fkN+fkj).norm()>FLT_EPSILON){
//							VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).connect(k,newID,fkN+fkj);
//						}
//					}
//					else {
//						VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).connect(k,newID,fkj);
//						
//					}					
//				}
//			}
//			
//			std::cout<<"I'm here 30"<<std::endl;
//
//			VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).template remove<1>(j); // removeIsolatedNodes is used here because annihilation leaves isolated nodes attached to j
//
//			
//			const isConstNetworkVertexType newVi(VertexFinder<VertexType>(networkVertexMapRef).node(i));
//			assert(!newVi.first && "CONTRACTING LEFT i");
//			const isConstNetworkVertexType newVj(VertexFinder<VertexType>(networkVertexMapRef).node(j));
//			assert(!newVj.first && "CONTRACTING LEFT j");
//			
//			if(VertexFinder<VertexType>(networkVertexMapRef).node(newID).second->is_isolated()){ // WHAT IF THE ONLY NEIGHBOR OF I IS J? THEN NEW_ID IS ISOLATED
//				std::cout<<"I'm here 40"<<std::endl;
//
//				VertexConnection<VertexType,EdgeType>(networkVertexMapRef,networkEdgeMapRef).template remove<0>(newID); 
//			}
////			return newID;
