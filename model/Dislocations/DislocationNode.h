...
#include <model/Network/NetworkTypedefs.h>
	...
	private:
		...
		void initMeshLocation(){
			if (shared.boundary_type){
				...
				if (SD.nodeMeshLocation == outsideMesh) std::cout<< "NODE_OUTSIDE_DOMAIN " << this->get_P().transpose() << std::endl;
			}
	...
	public:
		...
		bool is_removable() const {			
			// remove only nodes that are simple (1 in-link and 1 out-link, i.e. no junctions) and that have both links on same plane
			//return (this->is_simple() && planeNormals.size()==1); 
...
//	const double DislocationNode<dim,corder,InterpolationType,alpha,qOrder,QuadratureRule,MaterialType>::AB1=1.0/3.0;
	
	template <short unsigned int dim, short unsigned int corder, typename InterpolationType,
	/*	   */ double & alpha, short unsigned int qOrder, template <short unsigned int, short unsigned int> class QuadratureRule, 
	/*	   */ typename MaterialType>
	const double DislocationNode<dim,corder,InterpolationType,alpha,qOrder,QuadratureRule,MaterialType>::AB2=-16.0/12.0;
//	const double DislocationNode<dim,corder,InterpolationType,alpha,qOrder,QuadratureRule,MaterialType>::AB2= 1.0/3.0;
	
	template <short unsigned int dim, short unsigned int corder, typename InterpolationType,
	/*	   */ double & alpha, short unsigned int qOrder, template <short unsigned int, short unsigned int> class QuadratureRule, 
	/*	   */ typename MaterialType>
	const double DislocationNode<dim,corder,InterpolationType,alpha,qOrder,QuadratureRule,MaterialType>::AB3=5.0/12.0;
//	const double DislocationNode<dim,corder,InterpolationType,alpha,qOrder,QuadratureRule,MaterialType>::AB3= 1.0/3.0;
