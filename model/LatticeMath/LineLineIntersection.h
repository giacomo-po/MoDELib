...
namespace model
{
    ...
    enum LineLineIntersectionType{parallelLines=0,coincidentLines=1,nonIntersectingLines=2,intersectingLines=3,nonLatticeIntersection=4};
    ...
    struct LineLineIntersection
    {
        ...
        const std::pair<bool,int> intersectionType;
        const LatticeVector<3> P;
        
        
        static std::pair<bool,int> getIntersectionType(const LatticeVectorType& p12,
        ...
        {
            std::pair<bool,int> temp=std::make_pair(false,-1);
            if(d1Xd2.squaredNorm()==0) // parallel lines
            {
                ...
                if(dPXd1.squaredNorm()==0)
                {
                    ...
                    temp.first=true;
                    temp.second=coincidentLines;
                }
                else
                {
                    temp.first=false;
                    temp.second=parallelLines;
                }
            }
            else // non-parallel, non-coincident
            {
                ...
                {
                    temp.first=false;
                    temp.second=nonIntersectingLines;
                }
                else // planarity condition ok
                {
                    ...
                    {
                        temp.first=true;
                        temp.second=intersectingLines;
                    }
                    else
                    {
                        temp.first=false;
                        temp.second=nonLatticeIntersection;
                    }
                }
            }
            return temp;
        ...
        /* init */ P(intersectionType.second==coincidentLines? l1.P : (intersectionType.second==intersectingLines? (l1.P-dPxd2.gCD/d1xd2.gCD*(1-2*((dPxd2+d1xd2).squaredNorm()==0))*l1.d).eval() : VectorDimI::Zero()))
        {
        ...
        {
            os  <<lli.intersectionType.first<<" "<<lli.intersectionType.second<<"\n"
