...
//        double noiseSSMax(std::numeric_limits<double>::min());
    ...
    for(const auto& loopIO : configIO.loops())
    {
        ...
        if(glidePlane->slipSystems().size())
        {
            ...
            for(const auto& siIO : configIO.sphericalInclusions())
            {
            ...
            for(size_t slipSystemID=0; slipSystemID<poly.grain(loopIO.grainID).singleCrystal->slipSystems().size();++slipSystemID)
            {
                const auto& slipSystem(poly.grain(loopIO.grainID).singleCrystal->slipSystems()[slipSystemID]);
