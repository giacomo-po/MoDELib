...
{
    template <int dim>
    double SegmentSegmentDistance<dim>::limitRange(const double& v)
    {
        if(v<0.0)
        {
            return 0.0;
    ...
    template <int dim>
    std::pair<double,double> SegmentSegmentDistance<dim>::step4U(const double& T) const
    ...
    template <int dim>
    std::pair<double,double> SegmentSegmentDistance<dim>::step4(const double& U) const
    {/*! Compute t from eq. (10) and limit by eq. (12)
      */
    ...
    template <int dim>
    std::pair<double,double> SegmentSegmentDistance<dim>::step3(const double& T) const
    {
    ...
    template <int dim>
    std::pair<double,double> SegmentSegmentDistance<dim>::step2() const
    {
    ...
    template <int dim>
    std::pair<double,double> SegmentSegmentDistance<dim>::getTU() const
    {
        if(D1<tol && D2<tol)
        ...
        {// an intersection exists
            ...
            {// both segments are not degenerate
                ...
                {//  coincident segments. Since there must be an intersection, the two innermost points are the overlapping segment
                    ...
                    {
                        temp.emplace_back(0.5*(iter1->second+iter2->second),
