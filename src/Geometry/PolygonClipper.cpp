...
#ifndef model_PolygonClipper_CPP_
#define model_PolygonClipper_CPP_
#include <iostream>
#include <iomanip>
#include <fstream>
#include <string>
...
namespace model
{
    ...
    /******************************************************************************/
    LoopPathClipper::LoopPathClipper(const std::vector<typename LoopPathClipper::VectorDim>& loop,
                    const std::vector<typename LoopPathClipper::VectorDim>& patch)
    {
...
//         }
        ...
        for(auto iter=loopIntersections.begin();iter!=loopIntersections.end();++iter)
        {
            ...
            const typename LoopPathClipper::VectorDim midPoint(0.5*(*source+*sink));
    ...
    /******************************************************************************/
    void LoopPathClipper::makePaths()
    {
...
//Include a for loop for inserting the patch edges wn number of times
    ...
    /******************************************************************************/
    void LoopPathClipper::clipPolygon(const ClipperEdge *edg,
                    std::vector<const ClipperEdge *> &path,
                    std::set<const ClipperEdge *> &validStarts) const
    {
     ...
     /******************************************************************************/
    void LoopPathClipper::printRelevant()
    {
     ...
     /******************************************************************************/
     std::vector<std::vector<typename LoopPathClipper::VectorDim>> LoopPathClipper::getClippedPolygons()
     {
         std::vector<std::vector<typename LoopPathClipper::VectorDim>> nodeSeq;
         for (size_t k = 0; k < clippedPolygons().size(); ++k)
         {
             std::vector<typename LoopPathClipper::VectorDim> nodeSeqK;
