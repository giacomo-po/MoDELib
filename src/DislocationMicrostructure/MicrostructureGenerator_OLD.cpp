...
namespace model
{
    ...
    size_t MicrostructureGeneratorBase::insertLoop(const VectorDimD& b,const VectorDimD& unitNormal,const VectorDimD& P0,const size_t& grainID,const DislocationLoopType& loopType);
    {
    ...
    size_t MicrostructureGeneratorBase::insertNetworkpNode(const size_t& networkNodeID,const VectorDimD& networkNodePos)
    {
    ...
    size_t MicrostructureGeneratorBase::insertLoopNode(const size_t& loopID,const VectorDimD& loopNodePos,const size_t& networkNodeID,const VectorDimD& loopNodeShift,const std::pair<short int,short int>& periodicEdgeIDs)
    {
    ...
    bool MicrostructureGeneratorBase::addSingleLoop(const bool randomizeBurgersSense,
...
//                           const VectorDimD& periodicShift)
{
    ...
    assert(nodeIDs.size()==loopNodeIDs.size());
...
//                    const size_t nextNodeID=(k+1)<nodePos.size()? nodeID+k+1 : nodeID;
            const size_t currentLoopNodeID(loopNodeIDs[k]);
            const size_t nextLoopNodeID((k+1)<nodePos.size()? loopNodeIDs[k+1] : loopNodeIDs[0]);
            
//            const size_t currentNetworkNodeID(nodeIDs[k]);
//            const size_t nextNetworkNodeID((k+1)<nodeIDs.size()? loopNodeID+k+1 : loopNodeID);
...
//            configIO.loopNodes().emplace_back(loopNodeID+k,loopID,loopNodePos[k],nodeID+k,loopNodeShifts[k],periodicEdgeIDs[k]);
            configIO.loopLinks().emplace_back(loopID,currentLoopNodeID,nextLoopNodeID,(configIO.loopNodes()[nextLoopNodeID]-configIO.loopNodes()[currentLoopNodeID]).norm()>FLT_EPSILON,0);
        }
        configIO.loops().emplace_back(loopID, b,unitNormal,P0,grainID,loopType);
        nodeID+=nodePos.size();
...
/**********************************************************************/
void MicrostructureGeneratorBase::writeConfigFiles(const size_t& fileID)
{
...
//        }
        ...
        void MicrostructureGenerator::addIndividualPeriodicStraighDislocation()
        {
            
            const std::vector<int> periodicDipoleSlipSystemIDs(TextFileParser("./inputFiles/initialMicrostructure.txt").readArray<int>("periodicDipoleSlipSystemIDs",true));

            
//            /* init*/,straightDislocationsSlipSystemIDs
//            /* init*/,straightDislocationsAngleFromScrewOrientation
//            /* init*/,pointsAlongStraightDislocations
            ...
            if(periodicDipoleSlipSystemIDs.size())
            {
                ...
                const std::vector<int> periodicDipoleExitFaceIDs(TextFileParser("./inputFiles/initialMicrostructure.txt").readArray<int>("periodicDipoleExitFaceIDs",true));
                const Eigen::Matrix<double,Eigen::Dynamic,dim> periodicDipolePoints(TextFileParser("./inputFiles/initialMicrostructure.txt").readMatrix<double>("periodicDipolePoints",periodicDipoleSlipSystemIDs.size(),MicrostructureGenerator::dim,true));
                const std::vector<double> periodicDipoleHeights(TextFileParser("./inputFiles/initialMicrostructure.txt").readArray<double>("periodicDipoleHeights",true));
                
                if(periodicDipoleSlipSystemIDs.size()!=periodicDipoleExitFaceIDs.size())
                {
                    std::cout<<"(periodicDipoleSlipSystemIDs.size()!=periodicDipoleExitFaceIDs.size()).size()="<<periodicDipoleSlipSystemIDs.size()<<std::endl;
                    std::cout<<"periodicDipoleExitFaceIDs.size()="<<periodicDipoleExitFaceIDs.size()<<std::endl;
                    std::cout<<"You must provide one exit face ID for each dipole. EXITING."<<std::endl;
                    exit(EXIT_FAILURE);
                }
                if(int(periodicDipoleSlipSystemIDs.size())!=periodicDipolePoints.rows())
                {
                    std::cout<<"periodicDipoleSlipSystemIDs.size()="<<periodicDipoleSlipSystemIDs.size()<<std::endl;
                    std::cout<<"periodicDipolePoints.rows()="<<periodicDipolePoints.rows()<<std::endl;
                    std::cout<<"You must provide one point for each dipole. Each point is a row of the matrix periodicDipolePoints. EXITING."<<std::endl;
                    exit(EXIT_FAILURE);
                }
                if(int(periodicDipoleSlipSystemIDs.size())!=periodicDipoleHeights.size())
                {
                    std::cout<<"periodicDipoleSlipSystemIDs.size()="<<periodicDipoleSlipSystemIDs.size()<<std::endl;
                    std::cout<<"periodicDipoleHeights.size()="<<periodicDipoleHeights.size()<<std::endl;
                    std::cout<<"You must provide one dipole height for each dipole. EXITING."<<std::endl;
                    exit(EXIT_FAILURE);
                }
                ...
                for(size_t k=0;k<periodicDipoleSlipSystemIDs.size();++k)
                {
                    ...
                    if(rSS>=0)
                    {
                        std::pair<bool,const Simplex<dim,dim>*> found=mesh.search(periodicDipolePoints.row(k));
                        if(!found.first)
                        {
                        ...
                        assert(poly.grains().size()==1 && "Periodic dislocations only supported for single crystals");
                        const auto& grain(poly.grain(grainID));
                        ...
                        if(periodicFaceIter!=grain.region.faces().end())
                        {
                            ...
                            if(periodicFaceB!=nullptr)
                            {
                                ...
                                const auto faceAlatticeShift(poly.grain(grainID).latticeVector(faceAshift));
                                ...
                                if(slipSystem.n.dot(faceAlatticeShift)==0)
                                {
                                    ...
                                    std::shared_ptr<GlidePlane<3>> glidePlane(glidePlaneFactory.getFromKey(glidePlaneKey));
                                    ...
                                    if(pliA.type==PlaneLineIntersection<3>::INCIDENT && pliB.type==PlaneLineIntersection<3>::INCIDENT)
                                    {
                                        ...
                                        if((AB-faceAshift).norm()<FLT_EPSILON)
                                        {
                                            ...
                                            std::shared_ptr<GlidePlane<3>> parallelglidePlane(glidePlaneFactory.getFromKey(parallelGlidePlaneKey));
...
//                                            const VectorDimD hv()
                                            ...
                                            std::shared_ptr<PeriodicGlidePlane<3>> prismaticGlidePlane(pgpf.get(prismaticPlaneKey));
                                            ...
                                            for (const auto &tup : ppi)
                                            {
                                                const VectorDimD gblP(prismaticGlidePlane->referencePlane->globalPosition(std::get<0>(tup)));
                                                loopNodePos.push_back(gblP);
                                                networkNodePos.push_back(gblP + std::get<1>(tup));
                                                loopNodeShifts.push_back(std::get<1>(tup));
                                                edgeIDs.push_back(std::get<2>(tup));
                                            }
                                            addSingleLoop(false, networkNodePos, loopNodePos, slipSystem.s.cartesian(), prismaticGlidePlane->referencePlane->unitNormal, P0, grainID, DislocationLoopIO<dim>::SESSILELOOP, loopNodeShifts, edgeIDs);
//
//                                            //
//                                            if ()
//                                            {
//                                                density += 2.0 *AB.norm() / mesh.volume() / std::pow(poly.b_SI, 2);
//
...
//                                const VectorDimD P0=periodicDipolePoints.row(k).transpose()-periodicDipolePoints.row(k).dot(slipSystem.unitNormal)*slipSystem.unitNormal+slipSystem.unitNormal*slipSystem.n.planeSpacing()*heightPair.second;
...
//        /* init*/,mesh(meshFilename)
        ...
        /* init*/,pgpf(poly, glidePlaneFactory)
...
//            addEshelbyInclusions();
            this->writeConfigFiles(0,outputBinary);
