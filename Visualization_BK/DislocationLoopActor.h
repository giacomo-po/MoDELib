...
//        static float slippedAreaOpacity;
        ...
        std::map<std::pair<size_t,size_t>,DislocationSegmentIO<dim>> ddSegments;
...
//        vtkSmartPointer<vtkActor2D> singleNodeLabelActor;
        ...
        //        const std::map<size_t,const DislocationNodeIO<dim>* const>& configIO.nodeMap() const
        //        {
        //            return *this;
        //        }
        //
        //        std::map<size_t,const DislocationNodeIO<dim>* const>& configIO.nodeMap()
        //        {
        //            return *this;
        //        }
        //
...
//            colorVector=(clrVector*255).cast<int>();
        ...
        /**********************************************************************/
        DislocationLoopActor(vtkRenderer* const ren,
...
//        }
        ...
        void updateConfiguration(const DDconfigIO<3>& configIO)
        {
            ...
            // LoopLinks
            std::cout<<"Updating loopLinks..."<<std::flush;
    ...
    // Static data members
    float DislocationLoopActor::tubeRadius=5.0;
    bool DislocationLoopActor::scaleRadiusByBurgers=false;
    float DislocationLoopActor::alpha=0.5;
    DislocationLoopActor::ColorScheme DislocationLoopActor::clr=DislocationLoopActor::colorBurgers;
    size_t DislocationLoopActor::Np=2;
    bool DislocationLoopActor::showBoundarySegments=false;
    bool DislocationLoopActor::blackGrainBoundarySegments=false;
    bool DislocationLoopActor::showVelocities=false;
    bool DislocationLoopActor::showNodeIDs=false;
    float DislocationLoopActor::velocityFactor=100.0;
    bool DislocationLoopActor::showZeroBuergers=false;
    bool DislocationLoopActor::showSingleNode=false;
    size_t DislocationLoopActor::singleNodeID=0;
    bool DislocationLoopActor::showNodes=false;
    bool DislocationLoopActor::showSlippedArea=false;
    float DislocationLoopActor::slippedAreaOpacity=0.1;
} // namespace model
#endif
...
//                assert(edge.first.second.size());
//
//                auto itSource(configIO.nodeMap().find(edge.first.first)); //source
//                assert(itSource!=configIO.nodeMap().end() && "SOURCE VERTEX NOT FOUND IN V-FILE");
//                auto   itSink(configIO.nodeMap().find(edge.first.second)); //sink
//                assert(  itSink!=configIO.nodeMap().end() &&   "SINK VERTEX NOT FOUND IN V-FILE");
//
//                //                Eigen::Map<const Eigen::Matrix<double,1,6>> sourceRow(itSource->second.data());
//                //                Eigen::Map<const Eigen::Matrix<double,1,6>>   sinkRow(  itSink->second.data());
