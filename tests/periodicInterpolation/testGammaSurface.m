...
print([structure '_waveVectors'], '-depsc');
...
np=400;
printMatrixToFile(fid,[0:np-1]'/(np-1)*(A(1,1)+A(1,2)),'x');
printMatrixToFile(fid,[0:np-1]'/(np-1)*(A(2,1)+A(2,2)),'y');
fclose(fid)

system(['./test ' num2str(rotSymm) ' 1'])
wsc=load('output.txt'); % wave vector, S and C coeffs in each row
data=load('data.txt');
...
set(gca,'FontSize',fontSize)
print([structure '_gammaSurface'], '-depsc');
...
set(gca,'Fontsize',fontSize)
print([structure '_gammaSurfaceCuts'], '-depsc');
