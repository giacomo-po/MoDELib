...
const ClipperEdge* ClipperEdge::next(std::set<const ClipperEdge*>& validStarts) const
{
    ...
    else if (isPatchEdg)
    {
        ...
        if (sink->loopNext)
        {
            ...
            if (!sink->patchNext->twin() && sink->patchNext->wn!=0 )
            {
    ...
    else if (isReversePatchEdg)
    {
        ...
        {
            //Bifurcation exists INSERT....
            if (!sink->reversepatchNext->twin() && sink->reversepatchNext->wn!=0 )
            {
                validStarts.insert(sink->reversepatchNext);
            }
...
//                    std::cout<<t<<" "<<u<<std::endl;
    ...
    size_t totLoopstobeInserted()
    {
        size_t maxwn=0;
        for (const auto &edg : loopEdges())
        {
            if (maxwn<abs(edg.second.wn))
            {
                maxwn=abs(edg.second.wn);
            }
        }
        ...
        for (const auto &edg : reversepatchEdges())
        {
            if (maxwn<abs(edg.second.wn))
            {
                maxwn=abs(edg.second.wn);
    ...
    void makePaths()
        {
            ...
            if (!patchCompletelyInside)
            {
                ...
                // std::set<const ClipperEdge *> validStartsCopy;
                for (const auto &edg : loopEdges())
                {
                    if (edg.second.wn != 0 || edg.second.twin())
                    {
                        validStarts.insert(&edg.second);
                        // validStartsCopy.insert(&edg.second);
                    }
                }
            size_t maxPossible(totLoopstobeInserted());
            std::cout<<"MaxPOssible="<<maxPossible<<std::endl;
            bool clipperboolIterator(true);
                while (clipperboolIterator )
                {
                    ...
                    if (path.size() && clippedPolygons().size()<=maxPossible)
                    {
                    ...
                    if (clippedPolygons().size()>=maxPossible)
                    {
                        clipperboolIterator=false;
                    }
