...
//        enum {NdofXnode=NodeType::NdofXnode};
        ...
        public:
            DislocationGlideSolver(DislocationNetworkType &DN_in) :
             /*      init        */ DN(DN_in)
            {
                ...
                if (netNode.second.lock()->isBoundaryNode())
                {
                    ...
                    else
                    {
                        ...
                        for (const auto &loopConn : loopConnectivity)
                        {
                            ...
                            const auto nodeI(std::min(loopConn.first.first->networkNode->sID, loopConn.second.first->networkNode->sID) == loopConn.first.first->networkNode->sID ? std::make_pair(loopConn.first.first->networkNode.get(), loopConn.first.second) : std::make_pair(loopConn.second.first->networkNode.get(), loopConn.second.second));

                            const auto nodeJ(std::max(loopConn.first.first->networkNode->sID, loopConn.second.first->networkNode->sID) == loopConn.first.first->networkNode->sID ? std::make_pair(loopConn.first.first->networkNode.get(), loopConn.first.second) : std::make_pair(loopConn.second.first->networkNode.get(), loopConn.second.second));
