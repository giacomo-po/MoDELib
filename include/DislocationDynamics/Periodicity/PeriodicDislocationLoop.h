...
#ifndef NDEBUG
#define VerbosePeriodicDislocationBase(N,x) if(this->verbosePeriodicDislocationBase>=N){std::cout<<x;}
#else
...
//        PeriodicDislocationNodeType* const   sink;
    ...
    public:
                           ...
                           std::map<Eigen::Matrix<double, DislocationNetworkType::dim,1>, const std::shared_ptr<typename DislocationNetworkType::NodeType>, CompareVectorsByComponent<double,DislocationNetworkType::dim,float>>& rveNodesMap)
        {
            ...
            std::cout<<"        Initiating insertRVE loop with "<<points.size()<<" points ....."<<std::flush;
            ...
            std::cout<<"        Reinserting loop with "<<points.size()<<" points ("<<nodes.size()<<" nodes)"<<std::flush;
...
//            RVEnodeMapType nodeMap(getNodeMap());
            ...
            std::cout << magentaColor << std::setprecision(3) << std::scientific << " [" << (std::chrono::duration<double>(std::chrono::system_clock::now() - t6)).count() << " sec]." << defaultColor << std::endl;

            updateSharedNodeswithoutConstraints();
            // DN.io().output(50);


            std::cout<<"        Updating outer boundaries "<<std::flush;
            const auto t7= std::chrono::system_clock::now();
            updateOuterBoundaries();
            std::cout<<magentaColor<<std::setprecision(3)<<std::scientific<<outerBoundaries().size()<<" boundaries ["<<(std::chrono::duration<double>(std::chrono::system_clock::now()-t7)).count()<<" sec]."<<defaultColor<<std::endl;
            ...
            std::cout<<"        Clipping outer boundaries"<<std::flush;
...
//                }
            ...
            std::cout<<"        Removing "<<removeLoops.size()<<" loops"<<std::flush;
            ...
            std::cout<<"        Reinserting " <<reinsertVector.size()<<" loops."<<std::flush;
            for(const auto tup : reinsertVector)
            {// Re-insert in RVE
                insertRVEloop(DN,std::get<0>(tup),std::get<1>(tup),std::get<2>(tup),std::get<3>(tup),rveNodesMap);
            }
            std::cout<<magentaColor<<std::setprecision(3)<<std::scientific<<" ["<<(std::chrono::duration<double>(std::chrono::system_clock::now()-t3)).count()<<" sec]."<<defaultColor<<std::endl;
