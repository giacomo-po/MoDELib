...
#ifndef model_DislocationGlideSolver_hpp_
#define model_DislocationGlideSolver_hpp_
        ...
        for (const auto &netNode : DN.networkNodes())
        {
            ...
            if (netNode.second.lock()->isBoundaryNode())
            {
                for (const auto &loopNode : netNode.second.lock()->loopNodes())
                {
                    ...
                    {
                        const double lij = (loopNode->get_P() - loopNode->periodicPrev()->get_P()).norm();
                        const double ljk = (loopNode->get_P() - loopNode->periodicNext()->get_P()).norm();
                        
                        loopConnectivity.emplace(loopNode->loop().get(), std::make_pair(std::make_pair(loopNode->periodicPrev(), lij),
                                                                                        std::make_pair(loopNode->periodicNext(), ljk)));
                    }
                }
